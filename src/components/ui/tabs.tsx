...
function TabsList({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.List>) {
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn('inline-flex w-fit items-center justify-center rounded-lg bg-muted p-1', className)}
      {...props}
    />
  )
}
...
function TabsContent({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.Content>) {
  return (
    <TabsPrimitive.Content
      ...
      className={cn(
        'mt-4 outline-none focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:border-ring',
        className,
      )}
      {...props}
    />
